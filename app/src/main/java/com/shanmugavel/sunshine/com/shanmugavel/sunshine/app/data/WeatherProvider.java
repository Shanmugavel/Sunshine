...
public class WeatherProvider extends ContentProvider {
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (match) {
            ...
            case WEATHER_WITH_LOCATION : {
                ...
                values.put(WeatherContract.WeatherEntry.COLUMN_LOC_KEY, _loc_id);
                if (_loc_id > 0) {
                    long _weather_id = db.insert(WeatherContract.WeatherEntry.TABLE_NAME, null, values);
                    if ( _weather_id > 0 )
                        returnUri = WeatherContract.WeatherEntry.buildWeatherUri(_weather_id);
                    else
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case WEATHER: {
                deleteStatus = db.delete(WeatherContract.WeatherEntry.TABLE_NAME, selection, selectionArgs);
                break;
            }
            case WEATHER_WITH_LOCATION : {
                deleteStatus = db.delete(WeatherContract.WeatherEntry.TABLE_NAME, selection, selectionArgs);
            ...
            case LOCATION: {
                deleteStatus = db.delete(WeatherContract.LocationEntry.TABLE_NAME, selection, selectionArgs);
    ...
    public int update(
        ...
        switch (match) {
            case WEATHER: {
                normalizeDate(values);
                updateStatus = db.update(WeatherContract.WeatherEntry.TABLE_NAME, values, selection, selectionArgs);
                break;
            }
            case WEATHER_WITH_LOCATION : {
                normalizeDate(values);
            ...
            case LOCATION: {
                updateStatus = db.update(WeatherContract.LocationEntry.TABLE_NAME, values, selection, selectionArgs);
                break;
            }
            default:
                throw new UnsupportedOperationException("Unknown uri: " + uri);
